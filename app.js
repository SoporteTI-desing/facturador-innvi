...
function actualizarVistaImpresion(cliente, datosFactura) {
  ...
  const conceptosHtml = conceptosFactura
    .map((c, i) => {
      ...
      <tr>
        ...
        <td>${formatoMoneda(c.coaseguro || 0)}</td>
        <td>${formatoMoneda(c.deducible || 0)}</td>
        <td>${formatoMoneda(c.iva || 0)}</td>
      </tr>`;
    })
    .join("");
  ...
  <div class="factura-pdf">
    ...
    <div class="factura-pdf-totales">
      ...
      Total coaseguro: ${formatoMoneda(datosFactura.coaseguroTotal || 0)}<br/>
      Total deducible: ${formatoMoneda(datosFactura.deducibleTotal || 0)}<br/>
      Total CFDI: ${formatoMoneda(datosFactura.total)}<br/>
      <strong>Total paciente: ${formatoMoneda(datosFactura.totalPaciente != null ? datosFactura.totalPaciente : datosFactura.total)}</strong>
    </div>
